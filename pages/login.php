...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'register') {
    ...
    elseif (getUserByEmail($email)) {
      ...
      $user = getUserByEmail($email);
      $_SESSION['jb_user'] = ['id'=>$user['id'],'name'=>$user['name'],'email'=>$user['email']];
      header('Location: ' . BASE_URL . '/login.php?msg=welcome&name='.urlencode($name));
      exit;
    }
  }
...
<div class="auth-page">
  ...
  <div class="auth-visual">
    ...
    if (is_dir($imgDir)) {
      ...
      if (!empty($imgs)) $bgImg = $jbBase . '/assets/images/slider/' . $imgs[count($imgs) > 1 ? 1 : 0];
    }
    if ($bgImg): ?>
  ...
  <div class="auth-form-side">
    <div class="auth-form-wrap">
      ...
      <a href="<?= $jbBase ?>/index.php" class="auth-back-link">← Retour à la boutique</a>

      <?php if ($error): ?>
        <div class="auth-alert-error">⚠ <?= htmlspecialchars($error) ?>
        <?php if (!empty($showLoginLink)): ?>
          <a href="<?= $jbBase ?>/login.php?mode=login" style="color:var(--rose-deep);font-weight:600">Se connecter →</a>
        <?php endif; ?>
        </div>
      <?php endif; ?>
      <?php if ($success): ?>
      ...
      <div class="auth-switch">
        ...
        <a href="<?= $jbBase ?>/login.php?mode=register">Créer un compte gratuit →</a>
      </div>
