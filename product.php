...
if (!$product) { header('Location: /Jolly_Beauty/products.php'); exit; }
...
$mainImg = !empty($product['image']) ? '/Jolly_Beauty/images/'.basename($product['image']) : (isset($allImgs[0]) ? '/Jolly_Beauty/images/'.$allImgs[0] : null);
...
<div style="background:var(--rose-pale);padding:16px 6%;font-size:.76rem;color:var(--muted);" class="breadcrumb">
  <a href="/Jolly_Beauty/index.php">Accueil</a> ›
  <a href="/Jolly_Beauty/products.php?category=<?= urlencode($product['category'] ?? '') ?>"><?= ucfirst(sanitize($product['category'] ?? '')) ?></a> ›
  ...
  <div class="product-layout">
    ...
    <div class="product-gallery">
      ...
      <div class="gallery-thumbs">
        <?php foreach(array_slice($allImgs,0,4) as $i=>$img): ?>
        <img src="/Jolly_Beauty/images/<?= htmlspecialchars($img) ?>" class="gallery-thumb <?= $i===0?'active':'' ?>" onclick="switchImg(this,'/Jolly_Beauty/images/<?= htmlspecialchars($img) ?>')" alt="<?= sanitize($product['name']) ?>">
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
    </div>
    ...
    <div class="product-info">
      ...
      <div class="product-price">
        <?php if (!empty($product['original_price'])): ?><del><?= formatPrice($product['original_price']) ?></del><?php endif; ?>
...
<section class="bestsellers" style="background:var(--rose-pale);">
  ...
  <div class="products-grid">
    <?php foreach($related as $i => $p):
      $img = !empty($p['image']) ? '/Jolly_Beauty/images/'.basename($p['image']) : (isset($allImgs[$i]) ? '/Jolly_Beauty/images/'.$allImgs[$i] : null);
    ?>
    <div class="product-card">
      ...
      <div class="product-card__body">
        ...
        <div class="product-card__foot">
          ...
          <button class="product-card__add" onclick="addToCart({id:'<?= $p['id'] ?>',name:'<?= addslashes(sanitize($p['name'])) ?>',price:<?= floatval($p['price']) ?>,image:'<?= addslashes($img ?? '') ?>'})">+</button>
        </div>
      </div>
    </div>
    <?php endforeach; ?>
